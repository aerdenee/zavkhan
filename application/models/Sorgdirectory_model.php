...
class Sorgdirectory_model extends CI_Model {
    ...
    public function addFormData_model() {
        return array(
            ...
            'directory_web' => '',
            'directory_address' => '',
            'directory_social' => $this->socialDefault_model()
        );
    }
    ...
    public function lists_model($param = array('modId' => 0, 'catId' => 0)) {
        ...
        $this->data['html'] .= '<li>' . anchor(Sorgdirectory::$path . 'index/' . $param['modId'], '<i class="glyphicon glyphicon-chevron-left"></i> <i class="icon icon-primitive-dot" style="font-size:10px; margin-left:-5px;"></i><i class="icon icon-primitive-dot" style="font-size:10px;"></i><i class="icon icon-primitive-dot" style="font-size:10px;"></i>', array()) . '</li>';
        $this->data['html'] .= '</ul>';
        $this->data['html'] .= '</div>';
        $this->data['html'] .= '</div>';
        ...
        if ($this->auth->our->create == 1) {
            $this->data['html'] .= '<li style="padding-right:10px;">' . anchor(Sorgdirectory::$path . 'add/' . $param['modId'], '<i class="fa fa-plus"></i> Нэмэх', 'class="btn btn-info btn-rounded btn-xs"') . '</li>';
        } else {
        ...
        if ($this->resultCount > 0) {
            ...
            foreach ($this->result as $value) {
                ...
                if (($this->auth->our->update == 1 and $row['created_user_id'] == $this->session->adminUserId) or ( $this->auth->your->update == 1 and $row['created_user_id'] != $this->session->adminUserId)) {
                    $this->data['html'] .= '<li><a href="' . Sorgdirectory::$path . 'edit/' . $param['modId'] . '/' . $row['id'] . '"><i class="icon-pencil7"></i></a></li>';
                } else {
    ...
    public function insert_model($param = array()) {
        ...
        $this->param = $this->socialQuery = array();
        foreach ($this->input->post('social') as $key => $value) {
            $this->socialQuery[$key] = array('label' => $this->input->post('socialLabel[' . $key . ']'), 'address' => $this->input->post('social[' . $key . ']'), 'show' => $this->input->post('showSocial[' . $key . ']'));
        }
        ...
        $data = array(
            array(
                ...
                'directory_phone' => $this->input->post('directoryPhone'),
                'directory_fax' => $this->input->post('directoryFax'),
                'directory_email' => $this->input->post('directoryEmail'),
                'directory_web' => $this->input->post('directoryWeb'),
                'directory_address' => $this->input->post('directoryAddress'),
                'directory_social' => json_encode($this->socialQuery)
            )
        );
