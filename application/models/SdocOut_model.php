...
class SdocOut_model extends CI_Model {
    ...
    public function addFormData_model() {
        return json_decode(json_encode(array(
            ...
            'mod_id' => $this->modId,
            'doc_id' => 0,
            'from_department_id' => $this->session->userdata['adminDepartmentId'],
            'from_partner_id' => 0,
            'from_people_id' => 0,
            'to_department_id' => 0,
            'to_partner_id' => 0,
    ...
    public function listsCount_model($param = array()) {
        ...
        if ($param['toDepartmentId'] != 0) {

            $queryString .= ' AND DD.to_department_id IN (' . $this->hrPeopleDepartment->getChildHrPeopleDepartment_model($param['toDepartmentId']) . ')';
        }

        if ($param['toPartnerId'] != 0) {

            $queryString .= ' AND DD.to_partner_id IN (' . $this->hrPeopleDepartment->getChildHrPeopleDepartment_model($param['toPartnerId']) . ')';
        }

        if ($param['toPeopleId'] != 0) {

            $queryString .= ' AND DD.to_people_id IN (' . $this->hrPeopleDepartment->getChildHrPeopleDepartment_model($param['toPeopleId']) . ')';
        }
    ...
    public function lists_model($param = array('modId' => 0, 'catId' => 0, 'space' => '', 'path' => '')) {
        ...
        $query = $this->db->query('
            SELECT 
                ...
                DD.to_partner_id,
                DD.to_people_id,
                IF(DD.to_department_id = 51, P.title, HPD.title) AS to_department,
                DD.doc_close_id,
                DD.created_date,
                DD.modified_date,
                DD.created_user_id,
                ...
                END) AS row_status,
                D.page_number,
                IF(DD.doc_close_id > 0, CONCAT(CLOSE_DOC.doc_date, \' өдрийн \', CLOSE_DOC.doc_number, \' тоот албан бичгээр хариу ирсэн\'), \'\') AS close,
                CONCAT(SUBSTRING(U.lname, 1, 1), \'.\', U.fname) AS user
            FROM `gaz_doc_detail` AS DD
            INNER JOIN `gaz_doc` AS D ON DD.doc_id = D.id
            LEFT JOIN `gaz_master_doc_type` AS MDT ON D.doc_type_id = MDT.id
            LEFT JOIN `gaz_hr_people_department` AS HPD ON DD.to_department_id = HPD.id
            LEFT JOIN `gaz_partner` AS P ON DD.to_partner_id = P.id
            LEFT JOIN `gaz_hr_people` AS HP ON DD.to_people_id = HP.id
            LEFT JOIN `gaz_doc` AS CLOSE_DOC ON DD.doc_close_id = CLOSE_DOC.id
            ...
            foreach ($query->result() as $key => $row) {
                ...
                if ($row->is_read == 1) {
                    ...
                    array_push($data, array(
                        ...
                        'to_department' => '<strong>' . $row->to_department . '</strong>',
                        'created_user_id' => $row->created_user_id,
                        'description' => '<strong>' . $row->description . '</strong>',
                        'page_number' => '<strong>' . $row->page_number . '</strong>',
    ...
    public function insert_model($param = array()) {
        ...
        if ($doc['status'] == 'success') {
            ...
            foreach ($this->input->post('toDepartmentId') as $key => $toDepartmentId) {


                $childDepartment = explode(',', $this->hrPeopleDepartment->getChildHrPeopleDepartment_model($toDepartmentId));
                $childDepartmentCount = count($childDepartment);


                foreach ($childDepartment as $childRowToDepartment) {
                    if (($childDepartmentCount > 1 and $childRowToDepartment != $toDepartmentId) or ( $childDepartmentCount == 1 and $childRowToDepartment == $toDepartmentId)) {
                        ...
                        $dataDocDetail = array(
                            array(
                                ...
                                'to_department_id' => $childRowToDepartment,
                                'to_partner_id' => $this->input->post('toPartnerId[' . $key . ']'),
                                'to_people_id' => $this->input->post('toPeopleId[' . $key . ']'),
                                'created_date' => date('Y-m-d H:i:s'),
                                'modified_date' => date('Y-m-d H:i:s'),
                                'created_user_id' => $this->session->userdata['adminUserId'],
                                'modified_user_id' => $this->session->userdata['adminUserId'],
    ...
    public function update_model($param = array()) {
        ...
        $doc = $this->sdoc->update_model(array(
            ...
            'docTypeId' => $this->input->post('docTypeId'),
            'departmentId' => $this->input->post('fromDepartmentId'),
            'partnerId' => $this->input->post('fromPartnerId'),
            'peopleId' => $this->input->post('fromPeopleId'),
            'docDate' => $this->input->post('docDate'),
            'docNumber' => $this->input->post('docNumber'),
            'description' => $this->input->post('description'),
            'pageNumber' => $this->input->post('pageNumber'),
        ...
        if ($this->db->delete($this->db->dbprefix . 'doc_detail')) {
            ...
            $query = $this->db->query('
                SELECT 
                    DD.id
                FROM `gaz_doc_detail` AS DD
                WHERE DD.from_department_id = ' . $row->from_department_id);
    ...
    public function searchKeywordView_model($param = array('modId' => 0, 'path' => 'index')) {
        ...
        if ($this->input->get('docTypeId')) {
            ...
            $string .= form_hidden('createNumber', $this->input->get('docTypeId'));
            $showResetBtn = TRUE;
        }
        ...
        if ($this->input->get('toDepartmentId')) {
            $hrPeopleDepartment = $this->hrPeopleDepartment->getData_model(array('selectedId' => $this->input->get('toDepartmentId')));
            $string .= '<span class="label label-default label-rounded">' . $hrPeopleDepartment->title . '</span>';
            $string .= form_hidden('createNumber', $this->input->get('toDepartmentId'));
            $showResetBtn = TRUE;
        }

        if ($this->input->get('toPartnerId')) {
            $partner = $this->partner->getData_model(array('selectedId' => $this->input->get('toPartnerId')));
            $string .= '<span class="label label-default label-rounded">' . $partner->title . '</span>';
            $string .= form_hidden('createNumber', $this->input->get('toPartnerId'));
            $showResetBtn = TRUE;
        }

        if ($this->input->get('toPeopleId')) {
            $hrPeople = $this->hrPeople->getData_model(array('selectedId' => $this->input->get('toPeopleId')));
            $string .= '<span class="label label-default label-rounded">' . $hrPeople->title . '</span>';
            $string .= form_hidden('createNumber', $this->input->get('toPeopleId'));
            $showResetBtn = TRUE;
        }
        ...
        if ($showResetBtn) {
            $string .= ' <a href="javascript:;" onclick="_initDocOut({page: 0, searchQuery: {}});"><i class="icon-cancel-circle2 search-keyword-reset-btn"></i></a>';
        }

        return ($showResetBtn ? '<div class="_search-result-inner"><form action="javascript:;" id="form-doc-out"> Хайлтын үр дүн: ' . $string . '</form></div>' : '');
    }
