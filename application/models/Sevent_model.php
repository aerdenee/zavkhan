...
class Sevent_model extends CI_Model {
    ...
    public function lists_model($param = array('modId' => 0, 'catId' => 0)) {
        ...
        $this->data['html'] .= '<li>' . anchor(Sevent::$path . 'index/' . $param['modId'], '<i class="glyphicon glyphicon-chevron-left"></i> <i class="icon icon-primitive-dot" style="font-size:10px; margin-left:-5px;"></i><i class="icon icon-primitive-dot" style="font-size:10px;"></i><i class="icon icon-primitive-dot" style="font-size:10px;"></i>', array()) . '</li>';
        $this->data['html'] .= '</ul>';
        $this->data['html'] .= '</div>';
        $this->data['html'] .= '</div>';
        ...
        if ($this->auth->our->create == 1) {
            $this->data['html'] .= '<li style="padding-right:10px;">' . anchor(Sevent::$path . 'add/' . $param['modId'], '<i class="fa fa-plus"></i> Нэмэх', 'class="btn btn-info btn-rounded btn-xs"') . '</li>';
        } else {
        ...
        if ($this->resultCount > 0) {
            ...
            foreach ($this->result as $value) {
                ...
                if (($this->auth->our->update == 1 and $row['created_user_id'] == $this->session->adminUserId) or ( $this->auth->your->update == 1 and $row['created_user_id'] != $this->session->adminUserId)) {
                    $this->data['html'] .= '<li><a href="' . Sevent::$path . 'edit/' . $param['modId'] . '/' . $row['id'] . '"><i class="icon-pencil7"></i></a></li>';
                } else {
