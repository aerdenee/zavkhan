...
class SnifsWhere_model extends CI_Model {
    ...
    public function addFormData_model() {
        return json_decode(json_encode(array(
            ...
            'cat_id' => 0,
            'title' => '',
            'order_num' => getOrderNum(array('table' => 'nifs_where', 'field' => 'order_num')),
            'is_active' => 1,
    ...
    public function listsCount_model($param = array()) {
        ...
        if ($this->auth->our->read == 1 and $this->auth->your->read == 1) {
            ...
            $this->queryString .= ' AND NW.created_user_id = ' . $this->session->adminUserId;
        } else {
            $this->queryString .= ' AND NW.created_user_id = -1';
        }
        ...
        if ($param['keyword'] != '') {
            $this->queryString .= ' AND (LOWER(NW.title) LIKE LOWER(\'%' . $param['keyword'] . '%\'))';
        }
    ...
    public function lists_model($param = array('modId' => 0, 'catId' => 0, 'space' => '', 'path' => '')) {
        ...
        $this->query = $this->db->query('
            SELECT 
                ...
                NW.modified_user_id
            FROM `gaz_nifs_where` AS NW
            LEFT JOIN `gaz_category` AS C ON NW.cat_id = C.id
            WHERE 1 = 1 ' . $this->queryString . '
            ORDER BY NW.order_num DESC
        ...
        if ($this->auth->our->create == 1) {
            $this->html .= form_button('addNifsWhere', '<i class="icon-pencil7"></i> <span class="hidden-xs position-right">Шинэ (F2)</span>', 'class="btn btn-default legitRipple" onclick="_addFormNifsWhere({elem: this});"', 'button');
        } else {
            $this->html .= form_button('addNifsWhere', '<i class="icon-pencil7"></i> <span class="hidden-xs position-right">Шинэ (F2)</span>', 'class="btn btn-default legitRipple" disabled="true"', 'button');
        }

        $this->html .= form_button('searchNifsWhere', '<i class="icon-search4"></i> <span class="hidden-xs position-right">Хайх (F3)</span>', 'class="btn btn-default legitRipple" onclick="_advensedSearchNifsWhere({elem: this});"', 'button');
        ...
        if ($this->query->num_rows() > 0) {
            ...
            foreach ($this->query->result() as $key => $row) {
                ...
                $this->html .= '<td>' . ++$i . '</td>';
                $this->html .= '<td class="context-menu-nifs-where-selected-row" style="padding-left:' . $param['space'] . 'px;">' . $row->title . '</td>';
                $this->html .= '<td class="context-menu-nifs-where-selected-row">' . $row->cat_title . '</td>';
                $this->html .= '<td class="context-menu-nifs-where-selected-row text-center">' . ($row->is_active == 1 ? '<span class="label bg-teal dropdown-toggle"><span class="fa fa-check"></span></span>' : '<span class="label label-danger"><span class="fa fa-lock"></span></span>') . '</td>';
                ...
                if (($this->auth->our->update == 1 and $row->created_user_id == $this->session->adminUserId) or ( $this->auth->your->update == 1 and $row->created_user_id != $this->session->adminUserId)) {
                    $this->html .= '<li><a href="javascript:;" onclick="_editFormNifsWhere({elem: this, id:' . $row->id . '});" title="Засах" data-action="edit"></a></li>';
                } else {
                ...
                if (($this->auth->our->delete == 1 and $row->created_user_id == $this->session->adminUserId) or ( $this->auth->your->delete == 1 and $row->created_user_id != $this->session->adminUserId)) {
                    $this->html .= '<li><a href="javascript:;" onclick="_removeNifsWhere({elem: this, id:' . $row->id . '});" title="Устгах" data-action="delete"></a></li>';
                } else {
    ...
    public function insert_model($param = array('getUID' => 0)) {
        ...
        if ($this->db->insert_batch($this->db->dbprefix . 'nifs_where', $this->data)) {
    ...
    public function update_model($param = array('pic' => '')) {
        ...
        if ($this->db->update($this->db->dbprefix . 'nifs_where', $this->data)) {
    ...
    public function delete_model() {
        ...
        $this->db->where('id', $this->input->post('id'));
        if ($this->db->delete($this->db->dbprefix . 'nifs_where')) {
