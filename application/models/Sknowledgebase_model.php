...
class Sknowledgebase_model extends CI_Model {
    ...
    function __construct() {
        ...
        $this->load->model('Scategory_model', 'scategory');
    ...
    public function addFormData_model() {
        return json_decode(json_encode(array(
            ...
            'click_real' => 0,
            'is_active' => 1,
            'order_num' => getOrderNum(array('table' => 'knowledgebase', 'field' => 'order_num')),
            'show_social' => 1,
            'param' => '',
            'lang_id' => 0,
            'theme_layout_id' => 0,
    ...
    public function editFormData_model($param = array('selectedId' => 0)) {
        ...
        $this->query = $this->db->query('
            SELECT 
                ...
                C.show_pic_outside,
                C.show_pic_inside,
                (case when (C.pic is null or C.pic = \'\' or C.pic = \'default.svg\') then \'' . $this->simage->returnDefaultImage_model(array('table' => 'knowledgebase')) . '\' else concat(\'' . UPLOADS_CONTENT_PATH . 's_\', C.pic) end) as pic,
                C.show_title,
                C.title,
                C.link_title,
                C.intro_text,
...
//        }
    ...
    public function lists_model($param = array('modId' => 0, 'catId' => 0)) {
        ...
        $this->query = $this->db->query('
            SELECT 
                ...
                C.title,
                (case when (C.pic is null or C.pic = \'\') then "<img src=\"' . $this->simage->returnDefaultImage_model(array('table' => 'knowledgebase')) . '\">" else CONCAT("<img src=\"' . UPLOADS_CONTENT_PATH . CROP_SMALL . '", C.pic, "\">") end) as pic,
                C.order_num,
                C.created_date,
    ...
    public function insert_model($param = array()) {
        ...
        $data = array(
            array(
                ...
                'pic' => $this->input->post('knowledgebasePic'),
        ...
        if ($this->db->insert_batch($this->db->dbprefix . 'knowledgebase', $data)) {
    ...
    public function update_model($param = array()) {
        ...
        $this->data = array(
            ...
            'show_pic_outside' => $this->input->post('showPicOutside'),
            'show_pic_inside' => $this->input->post('showPicInside'),
            'pic' => ($this->input->post('knowledgebasePic') != '' ? $this->input->post('knowledgebasePic') : $this->input->post('knowledgebaseOldPic')),
            'show_title' => $this->input->post('showTitle'),
            'title' => $this->input->post('title'),
            'link_title' => $this->input->post('linkTitle'),
            'intro_text' => $this->input->post('introText'),
        ...
        if ($this->db->update($this->db->dbprefix . 'knowledgebase', $this->data)) {
            return array('status' => 'success', 'message' => 'Амжилттай хадгаллаа...');
        }
    ...
    public function delete_model() {
        ...
        $this->db->where('id', $this->input->post('id'));
        if ($this->db->delete($this->db->dbprefix . 'knowledgebase')) {
...
//        }
        ...
        if ($param['catId'] != 0) {
            $this->queryString .= ' AND C.cat_id IN( ' . $this->scategory->getChildCategores_model($param['catId']) . ')';
        }
        ...
        $this->query = $this->db->query('
            ...
            WHERE C.lang_id = ' . $this->session->adminLangId . $this->queryString);
...
//        }
        ...
        $this->html .= '<div class="container bg-white-300 rounded p-4">';
        

if ($this->query->num_rows() > 0) {

            $this->html .= '<h2 class="section-title">МЭДЛЭГИЙН САН</h2>';
        $this->html .= '<div class="section-description"></div>';
        $this->html .= '<ul class="features">';
            foreach ($this->query->result() as $key => $row) {

                $this->html .= '<li>
                    <div class="knowledgebase">
                        <i class="icon-' . $key . '"></i>
                        <h4 class="title"><a href="/sknowledgebase/show/' . $param['moduleMenuId'] . '/' . $row->id . '">' . $row->title . '</a></h4>
                        <div class="description">' . $row->intro_text . '</div>
                    </div>
                </li>';
    ...
    public function searchListCount_model($param = array('modId' => 0, 'catId' => 0)) {
...
//        }
        ...
        if ($param['keyword'] != '') {
            $this->queryString .= ' AND (LOWER(K.title) LIKE LOWER(\'%' . $param['keyword'] . '%\') OR LOWER(K.intro_text) LIKE LOWER(\'%' . $param['keyword'] . '%\'))';
        }

        $this->query = $this->db->query('
            SELECT 
                K.id
            FROM `gaz_knowledgebase` AS K
            WHERE K.lang_id = ' . $this->session->adminLangId . $this->queryString);

        return $this->query->num_rows();
    }

    public function searchList_model($param = array('modId' => 0, 'catId' => 0)) {
...
//        }
        ...
        $this->query = $this->db->query('
            SELECT 
                ...
                K.title,
                K.intro_text,
                (case when (K.pic is null or K.pic = \'\') then \'default.svg\' else concat(\'m_\', K.pic) end) as pic,
                K.click,
                COMM.comment_count,
                K.is_active_date,
                K.order_num,
                ...
                HPD.title AS department_title
            FROM `gaz_knowledgebase` AS K
            INNER JOIN `gaz_category` AS CAT ON K.cat_id = CAT.id
            INNER JOIN `gaz_user` AS U ON K.created_user_id = U.id
            INNER JOIN `gaz_hr_people` AS HP ON U.people_id = HP.id
            INNER JOIN `gaz_hr_people_department` AS HPD ON HP.department_id = HPD.id
            LEFT JOIN ( 
                SELECT 
                ...
                FROM `gaz_comment` AS COM 
                GROUP BY COM.cont_id, COM.mod_id 
            ) AS COMM ON K.mod_id = COMM.mod_id AND K.id = COMM.cont_id
            WHERE K.lang_id = ' . $this->session->adminLangId . $this->queryString . '
            ORDER BY K.order_num DESC
            LIMIT ' . $param['page'] . ', ' . $param['limit']);
        ...
        if ($this->query->num_rows() > 0) {
            ...
            $this->html .= '<span class="text-muted font-size-sm">Хайлтын үр дүн ' . $param['totalRows'] . ', (' . $this->benchmark->elapsed_time() . ' секунд)</span>';
            $this->html .= '<hr>';
            $this->html .= '<ul class="media-list mb-3">';
            foreach ($this->query->result() as $key => $row) {

                $this->html .= '<li class="media">
		                    		<div class="media-body">
			                    		<h6 class="media-title ml-0"><a href="sknowledgebase/show/' . $param['moduleMenuId'] . '/' . $row->id . '">' . $row->title . '</a></h6>
			                    		<ul class="list-inline list-inline-dotted text-muted mb-2">
			                    			<li class="list-inline-item"><a href="sknowledgebase/show/' . $param['moduleMenuId'] . '/' . $row->id . '" class="text-success">' . base_url('sknowledgebase/show/' . $param['moduleMenuId'] . '/' . $row->id) . '</a></li>
		                    				<li class="list-inline-item">Нийтэлсэн: ' . $row->author . '</li>
                                                                <li class="list-inline-item">Сэтгэгдэл ' . $row->comment_count . '</li>
			                    		</ul>
                                                        ' . $row->intro_text . '
		                    		</div>
		                    	</li>';
    ...
    public function showItemOtherList_model($param = array('modId' => 0, 'catId' => 0)) {
        ...
        if ($this->query->num_rows() > 0) {
            ...
            $i = 0;
            $this->html .= '<ul class="nav nav-tabs nav-tabs-solid nav-justified border-0">
                <li class="nav-item"><a href="#justified-right-icon-tab1" class="nav-link active" data-toggle="tab">Мэдлэгийн сан</a></li>
            </ul>';
            $this->html .= '<div class="tab-content">';
            $this->html .= '<div class="tab-pane fade show active" id="justified-right-icon-tab1">';
            $this->html .= '<ul class="list-news list-unstyled">';
            foreach ($this->query->result() as $key => $row) {
                ...
                $this->html .= '<li class="clearfix py-2">
                            <h1 class="mb-0">
                                <a href="sknowledgebase/show/' . $param['moduleMenuId'] . '/' . $row->id . '" title="' . $row->link_title . '">' . $row->title . '</a>
                            </h1>
                            <span>
                                <i class="icofont icofont-wall-clock"></i>
                                ' . dateDiff($row->is_active_date) . '
