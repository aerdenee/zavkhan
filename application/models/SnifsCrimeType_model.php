...
class SnifsCrimeType_model extends CI_Model {
    ...
    public function addFormData_model() {
        return json_decode(json_encode(array(
            ...
            'parent_id' => 0,
            'title' => '',
            'order_num' => getOrderNum(array('table' => 'nifs_crime_type', 'field' => 'order_num')),
            'is_active' => 1,
    ...
    public function controlNifsCrimeTypeDropdown_model($param = array('catId' => 0, 'selectedId' => 0)) {
        ...
        if ($this->query->num_rows() > 0) {
            ...
            foreach ($this->query->result() as $key => $row) {
                ...
                $this->html .= self::controlNifsCrimeTypeChildDropdown_model(array('selectedId' => $param['selectedId'], 'parentId' => $row->id, 'space' => '&nbsp; &nbsp;  &nbsp;  &nbsp; '));
            }
        }
    ...
    public function controlNifsCrimeTypeChildDropdown_model($param = array('parentId' => 0, 'selectedId' => 0, 'space' => '')) {
        ...
        if ($this->query->num_rows() > 0) {
            ...
            foreach ($this->query->result() as $key => $row) {
                ...
                $html .= self::controlNifsCrimeTypeChildDropdown_model(array('selectedId' => $param['selectedId'], 'parentId' => $row->id, 'space' => '&nbsp; &nbsp; &nbsp;  &nbsp; '));
            }
        }
    ...
    public function getChildNifsCrimeType_model($param = array()) {
        ...
        if (is_array($param) and count($param) > 0) {
            foreach ($param as $partnerKey => $partnerId) {
                $this->query = $this->db->query('
                    SELECT 
                        NCT.id
                    FROM `' . $this->db->dbprefix . 'nifs_crime_type` AS NCT 
