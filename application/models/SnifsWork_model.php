...
class SnifsWork_model extends CI_Model {

    function __construct() {
        parent::__construct();
    }

    public function controlWorkDropdown_model($param = array('selectedId' => 0)) {
        ...
        $this->query = $this->db->query('
            SELECT 
                id,
                title
            FROM `' . $this->db->dbprefix . 'nifs_work`
            WHERE is_active = 1
            ORDER BY order_num DESC');

        $this->html .= '<select name="workId" id="workId" class="select2" ' . $this->string . '>';
