...
class ShrPeoplePosition_model extends CI_Model {
    ...
    public function addFormData_model() {
        return json_decode(json_encode(array(
            ...
            'created_user_id' => 0,
            'modified_user_id' => 0,
            'order_num' => getOrderNum(array('table' => 'hr_people_position', 'field' => 'order_num'))
        )));
    }
    ...
    public function editFormData_model($param = array('id' => 0)) {
        ...
        $this->query = $this->db->query('
            SELECT 
                ...
                HPP.modified_date,
                HPP.created_user_id,
                HPP.modified_user_id,
                HPP.order_num
            FROM `gaz_hr_people_position` AS HPP
            WHERE HPP.id = ' . $param['id']);
    ...
    public function listsCount_model($param = array()) {
        ...
        if ($auth->our->read and $auth->your->read) {
            ...
            $queryString .= ' AND HPP.created_user_id = ' . $this->session->adminUserId;
        } else {
            $queryString .= ' AND HPP.created_user_id = -1';
        }

        if ($param['keyword'] != '') {
            $queryString .= ' AND (LOWER(HPP.title) LIKE LOWER(\'%' . $param['keyword'] . '%\'))';
        }
    ...
    public function lists_model($param = array('modId' => 0, 'catId' => 0, 'space' => '', 'path' => '')) {
        ...
        $query = $this->db->query('
            SELECT 
                ...
                HPP.mod_id,
                HPP.title,
                IF(HPP.is_active > 0, "<span class=\"badge badge-success\"><span class=\"fa fa-check\"></span></span>", "<span class=\"badge badge-danger\"><span class=\"fa fa-lock\"></span></span>") AS is_active,
                HPP.created_date,
                HPP.modified_date,
                HPP.created_user_id,
                HPP.modified_user_id,
    ...
    public function insert_model() {
        ...
        if ($this->db->insert_batch($this->db->dbprefix . 'hr_people_position', $this->data)) {
    ...
    public function update_model() {
        ...
        if ($this->db->update($this->db->dbprefix . 'hr_people_position', $data)) {
    ...
    public function delete_model() {
        $this->db->where('id', $this->input->post('id'));
        if ($this->db->delete($this->db->dbprefix . 'hr_people_position')) {
    ...
    public function searchKeywordView_model() {
        ...
        if ($this->showResetBtn) {
            ...
            $this->string .= ' <a href="javascript:;" onclick="_initHrPeoplePosition({page: 0, searchQuery: {}});"><i class="icon-cancel-circle2 search-keyword-reset-btn"></i></a>';
        }
    ...
    public function controlHrPeoplePositionDropDown_model($param = array('parentId' => 0, 'selectedId' => 0, 'name' => '')) {
