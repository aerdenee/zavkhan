...
class SnifsSceneFingerType_model extends CI_Model {
    ...
    public function addFormData_model() {
        return json_decode(json_encode(array(
            ...
            'parent_id' => 0,
            'title' => '',
            'order_num' => getOrderNum(array('table' => 'nifs_scene_finger_type', 'field' => 'order_num')),
            'is_active' => 1,
    ...
    public function editFormData_model($param = array('id' => 0)) {
        ...
        $this->query = $this->db->query('
            ...
            FROM `gaz_nifs_scene_finger_type` AS NCT
    ...
    public function lists_model($param = array('modId' => 0, 'catId' => 0, 'space' => '', 'path' => '')) {
        ...
        $this->html = form_open('', array('class' => 'form-horizontal', 'id' => 'form-nifs-scene-finger-type-init', 'enctype' => 'multipart/form-data'));
        ...
        if ($this->auth->our->create == 1) {
            $this->html .= form_button('addNifsSceneFingerType', '<i class="icon-pencil7"></i> <span class="hidden-xs position-right">Шинэ (F2)</span>', 'class="btn btn-default legitRipple" onclick="_addFormNifsSceneFingerType({elem: this});"', 'button');
        } else {
            $this->html .= form_button('addNifsSceneFingerType', '<i class="icon-pencil7"></i> <span class="hidden-xs position-right">Шинэ (F2)</span>', 'class="btn btn-default legitRipple" disabled="true"', 'button');
        }

        $this->html .= form_button('_searchNifsSceneFingerType', '<i class="icon-search4"></i> <span class="hidden-xs position-right">Хайх (F3)</span>', 'class="btn btn-default legitRipple" onclick="_advensedSearchNifsSceneFingerType({elem: this});"', 'button');
        ...
        if ($this->query->num_rows() > 0) {
            ...
            foreach ($this->query->result() as $key => $row) {
                ...
                $this->html .= '<td>' . ++$i . '</td>';
                $this->html .= '<td class="context-menu-nifs-scene-finger-type-selected-row" style="padding-left:' . $param['space'] . 'px;">' . $row->title . '</td>';
                $this->html .= '<td class="context-menu-nifs-scene-finger-type-selected-row">' . $row->cat_title . '</td>';
                $this->html .= '<td class="context-menu-nifs-scene-finger-type-selected-row">' . ($row->is_active == 1 ? '<span class="label bg-teal dropdown-toggle"><span class="fa fa-check"></span></span>' : '<span class="label label-danger"><span class="fa fa-lock"></span></span>') . '</td>';
                ...
                if (($this->auth->our->update == 1 and $row->created_user_id == $this->session->adminUserId) or ( $this->auth->your->update == 1 and $row->created_user_id != $this->session->adminUserId)) {
                    $this->html .= '<li><a href="javascript:;" onclick="_editFormNifsSceneFingerType({elem: this, id:' . $row->id . '});" title="Засах" data-action="edit"></a></li>';
                } else {
                ...
                if (($this->auth->our->delete == 1 and $row->created_user_id == $this->session->adminUserId) or ( $this->auth->your->delete == 1 and $row->created_user_id != $this->session->adminUserId)) {
                    $this->html .= '<li><a href="javascript:;" onclick="_removeNifsSceneFingerType({elem: this, id:' . $row->id . '});" title="Устгах" data-action="delete"></a></li>';
                } else {
    ...
    public function listsChild_model($param = array('parentId' => 0, 'space' => 10, 'autoNumber' => 1)) {
        ...
        if ($this->query->num_rows() > 0) {
            ...
            foreach ($this->query->result() as $row) {
                ...
                $html .= '<td class="context-menu-nifs-scene-finger-type-selected-row" style="padding-left:' . $param['space'] . 'px;">' . $row->title . '</td>';
                $this->html .= '<td class="context-menu-nifs-scene-finger-type-selected-row">' . ($row->is_active == 1 ? '<span class="label bg-teal dropdown-toggle"><span class="fa fa-check"></span></span>' : '<span class="label label-danger"><span class="fa fa-lock"></span></span>') . '</td>';
                $html .= '<td class="text-center">';
                $html .= '<ul class="icons-list">';

                if (($this->auth->our->update == 1 and $row->created_user_id == $this->session->adminUserId) or ( $this->auth->your->update == 1 and $row->created_user_id != $this->session->adminUserId)) {
                    $html .= '<li><a href="javascript:;" onclick="_editFormNifsSceneFingerType({elem: this, id:' . $row->id . '});" title="Засах" data-action="edit"></a></li>';
                } else {
                    $html .= '<li><a href="javascript:;" class="disabled" title="Засах" data-action="edit"></a></li>';
                }

                if (($this->auth->our->delete == 1 and $row->created_user_id == $this->session->adminUserId) or ( $this->auth->your->delete == 1 and $row->created_user_id != $this->session->adminUserId)) {
                    $html .= '<li><a href="javascript:;" onclick="_removeNifsSceneFingerType({elem: this, id:' . $row->id . '});" title="Устгах" data-action="delete"></a></li>';
                } else {
    ...
    public function insert_model($param = array('getUID' => 0)) {
        ...
        $this->data = array(
            array(
                'id' => getUID('nifs_scene_finger_type'),
        ...
        if ($this->db->insert_batch($this->db->dbprefix . 'nifs_scene_finger_type', $this->data)) {
    ...
    public function update_model($param = array('pic' => '')) {
        ...
        if ($this->db->update($this->db->dbprefix . 'nifs_scene_finger_type', $this->data)) {
    ...
    public function delete_model() {
        ...
        $delelteArray = $this->getChildNifsSceneFingerType_model($this->input->post('id'));
        $this->db->where_in('id', explode(',', $delelteArray));
        if ($this->db->delete($this->db->dbprefix . 'nifs_scene_finger_type')) {
    ...
    public function searchKeywordView_model() {
        ...
        if ($this->showResetBtn) {
            ...
            $this->string .= ' <a href="javascript:;" onclick="_initNifsSceneFingerType({page: 0, searchQuery: {}});"><i class="icon-cancel-circle2 search-keyword-reset-btn"></i></a>';
        }

        return $this->string;
    }

    public function controlNifsSceneFingerTypeDropdown_model($param = array('catId' => 0, 'selectedId' => 0)) {
        ...
        if ($this->query->num_rows() > 0) {
            ...
            foreach ($this->query->result() as $key => $row) {
                ...
                $this->html .= self::controlNifsSceneFingerTypeChildDropdown_model(array('selectedId' => $param['selectedId'], 'parentId' => $row->id, 'space' => '&nbsp; &nbsp;  &nbsp;  &nbsp; '));
            }
        }
    ...
    public function controlNifsSceneFingerTypeChildDropdown_model($param = array('parentId' => 0, 'selectedId' => 0, 'space' => '')) {
        ...
        $this->query = $this->db->query('
            SELECT 
                ...
                NCT.title
            FROM `gaz_nifs_scene_finger_type` AS NCT
            WHERE NCT.parent_id = ' . $param['parentId'] . ' AND NCT.is_active = 1
            ORDER BY NCT.order_num DESC');
        ...
        if ($this->query->num_rows() > 0) {
            ...
            foreach ($this->query->result() as $key => $row) {
                ...
                $html .= self::controlNifsSceneFingerTypeChildDropdown_model(array('selectedId' => $param['selectedId'], 'parentId' => $row->id, 'space' => '&nbsp; &nbsp; &nbsp;  &nbsp; '));
            }
        }
    ...
    public function getChildNifsSceneFingerType_model($param = array()) {
        ...
        if (is_array($param) and count($param) > 0) {
            foreach ($param as $partnerKey => $partnerId) {
                $this->query = $this->db->query('
                    SELECT 
                        NCT.id
                    FROM `' . $this->db->dbprefix . 'nifs_scene_finger_type` AS NCT 
                    WHERE NCT.is_active = 1 AND NCT.parent_id = ' . $partnerId);
            ...
            $this->query = $this->db->query('
                    SELECT 
                        NCT.id
                    FROM `' . $this->db->dbprefix . 'nifs_scene_finger_type` AS NCT 
                    WHERE NCT.is_active = 1 AND NCT.parent_id = ' . $param);
