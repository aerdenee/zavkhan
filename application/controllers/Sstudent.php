...
class Sstudent extends CI_Controller {

    public static $path = "sstudent/";
...
//' . $row->city_id . ', ' . $row->city_title . '
    ...
    public function listsStudentClass() {
        ...
        if ($this->session->isLogin === TRUE) {
            $this->body['mode'] = 'lists';
            $this->body['modId'] = $this->input->post('modId');
            $this->body['studentId'] = $this->input->post('studentId');
            ...
            if ($this->input->get('studentClassId') != '') {
                $this->urlString .= '&studentClassId=' . $this->input->get('studentClassId');
            }
            if ($this->input->get('keyword') != '') {
                $this->urlString .= '&keyword=' . $this->input->get('keyword');
            }
            ...
            $this->paginationConfig["base_url"] = base_url(self::$path . "index/" . $this->body['modId'] . ($this->urlString != '' ? '?' . $this->urlString : ''));
            $this->paginationConfig["total_rows"] = $this->student->listsCountStudentClass_model(array(
                'modId' => $this->body['modId'],
                'createdDate' => $this->input->get('createdDate'),
                'catId' => $this->input->get('catId'),
            ...
            echo json_encode(array(
                'mode' => $this->body['mode'],
                'title' => 'Анги бүртгэлийн хуудас',
                'html' => $this->student->listsStudentClass_model(array(
                    'rowCount' => $this->paginationConfig["total_rows"],
                    'path' => 'index'/* $this->body['path'] */,
                    'title' => $this->body['module']->title,
                    'modId' => $this->body['modId'],
    ...
    public function addStudentClass() {
        ...
        if ($this->session->isLogin === TRUE) {
            ...
            $this->header['jsFile'] = array('/assets/system/core/student.js');
            $this->body = array();
            $this->body['mode'] = 'insertStudentClass';
            $this->body['modId'] = ($this->uri->segment(3) != null ? $this->uri->segment(3) : $this->input->post('modId'));
            $this->body['studentId'] = $this->input->post('studentId');

            $this->body['module'] = $this->module->getData_model(array('id' => $this->body['modId']));
            $this->header['breadcrumb'] = $this->breadcrumb->breadcrumb_model(array('title' => $this->body['module']->title, 'mode' => $this->body['mode']));
            $this->body['row'] = $this->student->addStudentClassFormData_model();
            $this->page->deny_model(array('modId' => $this->body['modId'], 'mode' => $this->body['mode'], 'createdUserId' => $this->body['row']['created_user_id']));

            $this->body['controlAuthorDropdown'] = $this->user->controlAuthorDropdown_model(array('selectedId' => $this->body['row']['teacher_id'], 'name' => 'teacherId', 'isDisabled' => false));
            $this->body['controlClassListDropdown'] = $this->class->controlClassListDropdown_model(array('modId' => 37, 'selectedId' => $this->body['row']['teacher_id'], 'required' => true));
            $this->body['controlCategoryListDropdown'] = $this->category->controlCategoryListDropdown_model(array('modId' => $this->body['row']['mod_id'], 'selectedId' => $this->body['row']['cat_id'], 'parentId' => 0, 'space' => '', 'counter' => 1));
            $this->body['controlStatusListDropdown'] = $this->status->controlStatusListDropdown_model(array('modId' => $this->body['row']['mod_id'], 'selectedId' => $this->body['row']['status_id']));
            

            if ($this->input->is_ajax_request()) {
                echo json_encode(array(
                    'mode' => $this->body['mode'],
                    'title' => 'Анги бүртгэх хуудас',
                    'html' => $this->load->view(MY_ADMIN . '/student/formStudentClass', $this->body, TRUE),
                    'btn_ok' => 'Хадгалах',
                    'btn_no' => 'Хаах'));
            } else {
                $this->load->view(MY_ADMIN . '/header', $this->header);
                $this->load->view(MY_ADMIN . '/student/formStudentClass', $this->body);
                $this->load->view(MY_ADMIN . '/footer');
            }
        } else {
    ...
    public function editStudentClass() {

        if ($this->session->isLogin === TRUE) {
            $this->header['cssFile'] = array();
            $this->header['jsFile'] = array();
            $this->body = array();
            $this->body['mode'] = 'updateStudentClass';
            $this->body['modId'] = ($this->uri->segment(3) != null ? $this->uri->segment(3) : $this->input->post('modId'));
            $this->body['selectedId'] = ($this->uri->segment(4) != null ? $this->uri->segment(4) : $this->input->post('id'));
            $this->body['studentId'] = $this->input->post('studentId');
            ...
            $this->body['module'] = $this->module->getData_model(array('id' => $this->body['modId']));
            $this->header['breadcrumb'] = $this->breadcrumb->breadcrumb_model(array('title' => $this->body['module']->title, 'mode' => $this->body['mode']));
            $this->body['row'] = $this->student->editStudentClassFormData_model(array('id' => $this->body['selectedId']));
            $this->page->deny_model(array('modId' => $this->body['modId'], 'mode' => $this->body['mode'], 'createdUserId' => $this->body['row']['created_user_id']));

            $this->body['controlAuthorDropdown'] = $this->user->controlAuthorDropdown_model(array('selectedId' => $this->body['row']['teacher_id'], 'name' => 'teacherId', 'isDisabled' => false));
            $this->body['controlClassListDropdown'] = $this->class->controlClassListDropdown_model(array('modId' => 37, 'selectedId' => $this->body['row']['class_id'], 'required' => true));
            $this->body['controlStatusListDropdown'] = $this->status->controlStatusListDropdown_model(array('modId' => $this->body['row']['mod_id'], 'selectedId' => $this->body['row']['status_id']));
    ...
    public function insertStudentClass() {
        ...
        $this->getUID = getUID('student_class');

        echo json_encode($this->student->insertStudentClass_model(array('getUID' => $this->getUID)));
    }

    public function updateStudentClass() {
        echo json_encode($this->student->updateStudentClass_model());
    }

    public function deleteStudentClass() {
        echo json_encode($this->student->deleteStudentClass_model());
    }

    public function listsStudentFinance() {
        $this->urlString = '';
        if ($this->session->isLogin === TRUE) {
            $this->body['mode'] = 'lists';
            $this->body['modId'] = $this->input->post('modId');
            ...
            echo json_encode(array(
                'mode' => $this->body['mode'],
                'title' => 'Анги бүртгэлийн хуудас',
                'html' => $this->student->listsStudentFinance_model(array(
                    'title' => $this->body['module']->title,
                    'modId' => $this->body['modId'],
    ...
    public function addStudentFinance() {
        ...
        if ($this->session->isLogin === TRUE) {
            ...
            $this->header['jsFile'] = array('/assets/system/core/student.js');
            $this->body = array();
            $this->body['mode'] = 'insertStudentFinance';
            $this->body['modId'] = $this->input->post('modId');
            $this->body['studentId'] = $this->input->post('studentId');
            ...
            $this->body['module'] = $this->module->getData_model(array('id' => $this->body['modId']));
            $this->header['breadcrumb'] = $this->breadcrumb->breadcrumb_model(array('title' => $this->body['module']->title, 'mode' => $this->body['mode']));
            $this->body['row'] = $this->student->addStudentFinanceFormData_model();
            $this->page->deny_model(array('modId' => $this->body['modId'], 'mode' => $this->body['mode'], 'createdUserId' => $this->body['row']['created_user_id']));

            $this->body['controlClassListDropdown'] = $this->class->controlClassListDropdown_model(array('modId' => 37, 'selectedId' => $this->body['row']['class_id'], 'studentId' => $this->body['studentId']));
            $this->body['classGetData'] = $this->class->getData_model(array('selectedId' => $this->body['row']['class_id']));
            $this->body['studentFinanceSum'] = $this->student->studentFinanceSum_model(array('studentId' => $this->body['row']['student_id'], 'classId' => $this->body['row']['class_id']));

            if ($this->input->is_ajax_request()) {
                echo json_encode(array(
                    'mode' => $this->body['mode'],
                    'title' => 'Анги бүртгэх хуудас',
                    'html' => $this->load->view(MY_ADMIN . '/student/formStudentFinance', $this->body, TRUE),
                    'btn_ok' => 'Хадгалах',
                    'btn_no' => 'Хаах'));
            } else {
                $this->load->view(MY_ADMIN . '/header', $this->header);
    ...
    public function editStudentFinance() {

        if ($this->session->isLogin === TRUE) {
            $this->header['cssFile'] = array();
            $this->header['jsFile'] = array();
            $this->body = array();
            $this->body['mode'] = 'updateStudentFinance';
            $this->body['modId'] = ($this->uri->segment(3) != null ? $this->uri->segment(3) : $this->input->post('modId'));
            $this->body['selectedId'] = ($this->uri->segment(4) != null ? $this->uri->segment(4) : $this->input->post('id'));
            $this->body['studentId'] = $this->input->post('studentId');
            ...
            $this->body['module'] = $this->module->getData_model(array('id' => $this->body['modId']));
            $this->header['breadcrumb'] = $this->breadcrumb->breadcrumb_model(array('title' => $this->body['module']->title, 'mode' => $this->body['mode']));
            $this->body['row'] = $this->student->editStudentFinanceFormData_model(array('id' => $this->body['selectedId']));
            $this->page->deny_model(array('modId' => $this->body['modId'], 'mode' => $this->body['mode'], 'createdUserId' => $this->body['row']['created_user_id']));

            $this->body['controlClassListDropdown'] = $this->class->controlClassListDropdown_model(array('modId' => 37, 'selectedId' => $this->body['row']['class_id'], 'studentId' => $this->body['studentId'], 'required' => true));
            $this->body['classGetData'] = $this->class->getData_model(array('selectedId' => $this->body['row']['class_id']));
            $this->body['studentFinanceSum'] = $this->student->studentFinanceSum_model(array('studentId' => $this->body['row']['student_id'], 'classId' => $this->body['row']['class_id']));
            ...
            if ($this->input->is_ajax_request()) {
                ...
                $this->load->view(MY_ADMIN . '/header', $this->header);
                $this->load->view(MY_ADMIN . '/student/formStudentFinance', $this->body);
                $this->load->view(MY_ADMIN . '/footer');
            }
        } else {
            redirect(MY_ADMIN);
    ...
    public function studentFinanceSum() {
        echo json_encode($this->student->studentFinanceSum_model(array('studentId' => $this->input->post('studentId'), 'classId' => $this->input->post('classId'))));
    }
    
    public function insertStudentFinance() {

        $this->getUID = getUID('student_finance');

        echo json_encode($this->student->insertStudentFinance_model(array('getUID' => $this->getUID)));
    }

    public function updateStudentFinance() {
        echo json_encode($this->student->updateStudentFinance_model());
    }

    public function deleteStudentFinance() {
        echo json_encode($this->student->deleteStudentFinance_model());
    }
}
