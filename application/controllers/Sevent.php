...
class Sevent extends CI_Controller {

    public static $path = "sevent/";

    function __construct() {
        parent::__construct();
        $this->load->model('Spage_model', 'page');
        $this->load->model('Sevent_model', 'event');
    ...
    public function index() {
        ...
        if ($this->session->isLogin === TRUE) {
            ...
            $this->header['jsFile'] = array('/assets/system/core/event.js', '/assets/system/core/map.js');
            $this->body = array();
            ...
            $config["total_rows"] = $this->event->listsCount_model(array('modId' => $this->body['modId'], 'keyword' => $this->input->get('keyword'), 'catId' => $this->input->get('catId'), 'organizationId' => $this->input->get('campId')));
            ...
            $this->body['dataHtml'] = $this->event->lists_model(array(
            ...
            $this->load->view(MY_ADMIN . '/event/index', $this->body);
    ...
    public function add() {
        ...
        if ($this->session->isLogin === TRUE) {
            ...
            $this->header['cssFile'] = array();
            $this->header['jsFile'] = array('/assets/editor/ckeditor/ckeditor.js', '/assets/system/core/event.js', '/assets/system/core/map.js');
            $this->body = array();
            ...
            $this->body['row'] = $this->event->addFormData_model();
            ...
            $this->load->view(MY_ADMIN . '/header', $this->header);
            $this->load->view(MY_ADMIN . '/event/formGeneral', $this->body);
            $this->load->view(MY_ADMIN . '/footer');
        } else {
            redirect(MY_ADMIN);
        }
    ...
    public function edit() {
        ...
        if ($this->session->isLogin === TRUE) {
            ...
            $this->body['row'] = $this->event->editFormData_model(array('id' => $this->uri->segment(4)));
    ...
    public function insert() {
        ...
        echo json_encode($this->event->insert_model(array('pic' => $this->newFile, 'getUID' => $this->getUID)));
    }
    ...
    public function update() {
        ...
        echo json_encode($this->event->update_model(array('pic' => $this->newFile)));
    }

    public function isActive() {
        echo json_encode($this->event->isActive_model());
    }

    public function delete() {
        echo json_encode($this->event->delete_model());
    }
    ...
    public function updateVerticalImage() {
        ...
        echo json_encode($this->event->updateVerticalImage_model(array('pic' => $this->newFile, 'id' => $this->input->post('id'))));
    }
    ...
    public function mediaForm() {
        ...
        $data['row'] = $this->event->mediaAddFormData_model();
        if ($this->input->post('mode') == 'mediaUpdate') {
            $data['row'] = $this->event->mediaEditFormData_model(array('id' => $this->input->post('mediaId')));
        }

        echo json_encode(
                array(
                    "title" => "Медиа файл",
                    "html" => $this->load->view(MY_ADMIN . '/event/formMedia', $data, true),
                    "btn_ok" => "Хадгалах",
                    "btn_no" => "Хаах"));
    }
    ...
    public function mediaInsert() {
        ...
        if ($this->event->mediaInsert_model(array('fileNameMn' => $this->newFile['nameMn'], 'fileNameEn' => $this->newFile['nameEn'], 'fileTypeMn' => $this->newFile['fileTypeMn'], 'fileTypeEn' => $this->newFile['fileTypeEn']))) {
    ...
    public function mediaUpdate() {
        ...
        if ($this->event->mediaUpdate_model(array('fileNameMn' => $this->newFile['nameMn'], 'fileNameEn' => $this->newFile['nameEn'], 'fileTypeMn' => $this->newFile['fileTypeMn'], 'fileTypeEn' => $this->newFile['fileTypeEn']))) {
            echo json_encode(array('status' => 'success', 'message' => 'Амжилттай хадгаллаа...'));
        } else {
            echo json_encode(array('status' => 'error', 'message' => 'Хадгалах үед алдаа гарлаа...'));
        }
    ...
    public function mediaList() {
        echo json_encode($this->event->mediaList_model(array('modId' => $this->input->post('modId'), 'contId' => $this->input->post('contId'), 'type' => $this->input->post('type'))));
    }

    public function mediaDelete() {
        echo json_encode($this->event->mediaDelete_model());
    }
    ...
    function searchForm() {
        ...
        echo json_encode(array(
            ...
            'html' => $this->load->view(MY_ADMIN . '/event/formSearch', $this->body, TRUE),
            'btn_yes' => 'Хайх',
            'btn_no' => 'Болих'
        ));
    }
    ...
    public function tourBackground() {
        echo json_encode($this->event->tourBackground_model());
    }

    public function tourIncludedService() {
        echo json_encode($this->event->tourIncludedService_model());
    }

    public function tourItineraryList() {
        echo json_encode($this->event->tourItineraryList_model(array('modId' => $this->input->post('modId'), 'contId' => $this->input->post('contId'))));
    }
    ...
    public function formTourItinerary() {
        ...
        $this->data['type'] = 5;
        $this->data['row'] = $this->event->mediaAddFormData_model();
        if ($this->input->post('mode') == 'tourItineraryUpdate') {
            $this->data['row'] = $this->event->mediaEditFormData_model(array('id' => $this->input->post('mediaId')));
        }

        echo json_encode(
                array(
                    "title" => "Tour itinerary",
                    "html" => $this->load->view(MY_ADMIN . '/event/formTourItinerary', $this->data, true),
                    "btn_ok" => "Хадгалах",
                    "btn_no" => "Хаах"));
    }

    public function tourItineraryInsert() {
        if ($this->event->tourItineraryInsert_model()) {
    ...
    public function tourItineraryUpdate() {
        if ($this->event->tourItineraryUpdate_model()) {
            echo json_encode(array('status' => 'success', 'title' => 'Амжилттай', 'message' => 'Амжилттай хадгаллаа...'));
        } else {
            echo json_encode(array('status' => 'error', 'title' => 'Алдаа', 'message' => 'Хадгалах үед алдаа гарлаа...'));
        }
    ...
    public function tourItineraryDelete() {
        echo json_encode($this->event->tourItineraryDelete_model());
    }
    ...
    public function timeLineForm() {
        ...
        $data['modId'] = $this->input->post('modId');
        $data['mediaId'] = $this->input->post('mediaId');
        $data['row'] = $this->event->timeLineAddFormData_model();
        if ($this->input->post('mode') == 'timeLineUpdate') {
            $data['row'] = $this->event->timeLineEditFormData_model(array('id' => $this->input->post('mediaId')));
        }

        echo json_encode(
                array(
                    "title" => "Медиа файл",
                    "html" => $this->load->view(MY_ADMIN . '/event/formTimeLine', $data, true),
                    "btn_ok" => "Хадгалах",
                    "btn_no" => "Хаах"));
    }
    ...
    public function timeLineInsert() {
        ...
        if ($this->event->timeLineInsert_model(array('fileNameMn' => $this->newFileMn, 'fileNameEn' => $this->newFileEn))) {
            echo json_encode(array('status' => 'success', 'title' => 'Амжилттай', 'message' => 'Амжилттай хадгаллаа...'));
        } else {
            echo json_encode(array('status' => 'error', 'title' => 'Алдаа', 'message' => 'Хадгалах үед алдаа гарлаа...'));
        }
    ...
    public function timeLineUpdate() {
        ...
        if ($this->event->timeLineUpdate_model(array('fileNameMn' => $this->newFileMn, 'fileNameEn' => $this->newFileEn))) {
            echo json_encode(array('status' => 'success', 'title' => 'Амжилттай', 'message' => 'Амжилттай хадгаллаа...'));
        } else {
            echo json_encode(array('status' => 'error', 'title' => 'Алдаа', 'message' => 'Хадгалах үед алдаа гарлаа...'));
        }
    ...
    public function timeLineList() {
        echo json_encode($this->event->timeLineList_model(array('modId' => $this->input->post('modId'), 'contId' => $this->input->post('contId'), 'type' => $this->input->post('type'))));
    }

    public function timeLineDelete() {
        echo json_encode($this->event->timeLineDelete_model());
    }
    ...
    public function ourTeamForm() {
        ...
        $data['modId'] = $this->input->post('modId');
        $data['mediaId'] = $this->input->post('mediaId');
        $data['row'] = $this->event->ourTeamAddFormData_model();
        if ($this->input->post('mode') == 'ourTeamUpdate') {
            $data['row'] = $this->event->ourTeamEditFormData_model(array('id' => $this->input->post('mediaId')));
        }

        echo json_encode(
                array(
                    "title" => "Медиа файл",
                    "html" => $this->load->view(MY_ADMIN . '/event/formOurTeam', $data, true),
                    "btn_ok" => "Хадгалах",
                    "btn_no" => "Хаах"));
    }
    ...
    public function ourTeamInsert() {
        ...
        echo json_encode($this->event->ourTeamInsert_model(array('fileNameMn' => $this->newFile, 'fileNameEn' => $this->newFile)));
    }
    ...
    public function ourTeamUpdate() {
        ...
        if ($this->event->ourTeamUpdate_model(array('fileNameMn' => $this->newFile, 'fileNameEn' => $this->newFile))) {
            echo json_encode(array('status' => 'success', 'title' => 'Амжилттай', 'message' => 'Амжилттай хадгаллаа...'));
        } else {
            echo json_encode(array('status' => 'error', 'title' => 'Алдаа', 'message' => 'Хадгалах үед алдаа гарлаа...'));
        }
    ...
    public function ourTeamList() {
        echo json_encode($this->event->ourTeamList_model(array('modId' => $this->input->post('modId'), 'contId' => $this->input->post('contId'), 'type' => $this->input->post('type'))));
    }

    public function ourTeamDelete() {
        echo json_encode($this->event->ourTeamDelete_model());
    }
    ...
    public function attachFileForm() {
        ...
        $data['modId'] = $this->input->post('modId');
        $data['mediaId'] = $this->input->post('mediaId');
        $data['row'] = $this->event->attachFileAddFormData_model();
        if ($this->input->post('mode') == 'attachFileUpdate') {
            $data['row'] = $this->event->attachFileEditFormData_model(array('id' => $this->input->post('mediaId')));
        }

        echo json_encode(
                array(
                    "title" => "Хавсралт файл",
                    "html" => $this->load->view(MY_ADMIN . '/event/formAttachFile', $data, true),
                    "btn_ok" => "Хадгалах",
                    "btn_no" => "Хаах"));
    }
    ...
    public function attachFileInsert() {
        ...
        if ($this->event->attachFileInsert_model(array('fileNameMn' => $this->newFileMn, 'fileNameEn' => $this->newFileEn))) {
            echo json_encode(array('status' => 'success', 'title' => 'Амжилттай', 'message' => 'Амжилттай хадгаллаа...'));
        } else {
            echo json_encode(array('status' => 'error', 'title' => 'Алдаа', 'message' => 'Хадгалах үед алдаа гарлаа...'));
        }
    ...
    public function attachFileUpdate() {
        ...
        if ($this->event->attachFileUpdate_model(array('fileNameMn' => $this->newFileMn, 'fileNameEn' => $this->newFileEn))) {
            echo json_encode(array('status' => 'success', 'title' => 'Амжилттай', 'message' => 'Амжилттай хадгаллаа...'));
        } else {
            echo json_encode(array('status' => 'error', 'title' => 'Алдаа', 'message' => 'Хадгалах үед алдаа гарлаа...'));
        }
    ...
    public function attachFileList() {
        echo json_encode($this->event->attachFileList_model(array('modId' => $this->input->post('modId'), 'contId' => $this->input->post('contId'), 'type' => $this->input->post('type'))));
    }

    public function attachFileDelete() {
        echo json_encode($this->event->attachFileDelete_model());
    }
