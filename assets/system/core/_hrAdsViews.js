$(document).ready(function () {
    ...
    if (part[2] == 'index') {
        _initHrAdsViews({page: 0, searchQuery: {}});
    }
...
function _initHrAdsViews(param) {
    $.ajax({
        type: 'get',
        url: _hrAdsViewsModRootPath + 'lists',
...
function _showLearning(param) {
    $.ajax({
        type: 'get',
        url: _hrAdsViewsModRootPath + 'show',
        ...
        beforeSend: function () {
            ...
            window.location.href = _hrAdsViewsModRootPath + 'show/' + param.selectedId;
            $.blockUI({
                message: _jqueryBlockUiMessage,
                overlayCSS: _jqueryBlockUiOverlayCSS,
                css: _jqueryBlockUiMessageCSS
...
function _advensedSearchContent(param) {
    ...
    $.ajax({
        url: _hrAdsViewsModRootPath + 'searchForm',
        type: 'POST',
        dataType: 'json',
            ...
            $('#' + _dialogId).dialog({
                ...
                buttons: [
                    ...
                    {text: data.btn_yes, class: 'btn btn-primary active legitRipple', click: function () {
                            ...
                            _initHrAdsViews({page: 0, searchQuery: $('#' + _dialogId).find('form').serialize()});
                            $('#' + _dialogId).empty().dialog('close');
                            $.unblockUI();
                        }}
                ]
